...
(function () {
	...
	var Debug      = window.SphereXRDebug || {
		enabled: DEBUG,
		log: function () {
	...
	if (!Core) {
		Debug.error('[SphereXR] Core not loaded - engine aborted.');
		return;
	}
	...
	function initAnimation(el, cfg) {
		...
		function resize() {
			...
			if (DEBUG && state.h <= 1) {
				Debug.warn('[SphereXR] #' + cfg.animation_id + ' has height ' + state.h + 'px. Set a min-height on the container (Elementor -> Advanced -> Min Height, or CSS --spherexr-min-height).');
			}
		}

		function logResizeDebug() {
			if (DEBUG) {
				Debug.log('[SphereXR frontend] #' + cfg.animation_id + ' resize', getInteractionDebugState());
			}
		}
		...
		requestAnimationFrame(function () {
			...
			Debug.log('[SphereXR] Initialized #' + cfg.animation_id, {
				orbs: (cfg.orbs || []).length,
